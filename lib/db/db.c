...
int fd_db_open(void **stream, int mode, char const *path)
{
    ...
    switch (mode) {
    case FD_DBOM_READ: {
        errcode = fopen_s(*stream, path, "rb+");
    } break;
    case FD_DBOM_WRITE: {
        errcode = fopen_s(*stream, path, "wb+");
    } break;
    default: {
...
int fd_db_read(void **stream, struct fd_db_all *data_ptr)
{
    int retval = FD_DBERR_SUCCESS;
...
#pragma clang diagnostic ignored "-Wdollar-in-identifier-extension"
    ...
    if ((retval = db$read_header(stream, header)) != FD_DBERR_SUCCESS)
        return retval;
    ...
    if ((retval = db$read_config(stream, config)) != FD_DBERR_SUCCESS)
        return retval;
    ...
    for (size_t i = 0; i < header->dirs_count; ++i) {
        if ((retval = db$read_dir(stream, &dirs[i])) != FD_DBERR_SUCCESS)
            return retval;
    }
    ...
    for (size_t i = 0; i < header->files_count; ++i) {
        if ((retval = db$read_file(stream, &files[i])) != FD_DBERR_SUCCESS)
            return retval;
    }
...
int fd_db_write(void **stream, struct fd_db_all const *data)
{
    int retval = FD_DBERR_SUCCESS;
    ...
    // Write header
    if ((retval = db$write_header(stream, header)) != FD_DBERR_SUCCESS)
        return retval;

    // Write config
    if ((retval = db$write_config(stream, config)) != FD_DBERR_SUCCESS)
        return retval;

    // Write directory entries
    for (; dirs != dirs_end; ++dirs) {
        if ((retval = db$write_dir(stream, dirs)) != FD_DBERR_SUCCESS)
            return retval;
    }

    // Write file entries
    for (; files != files_end; ++files) {
        if ((retval = db$write_file(stream, files)) != FD_DBERR_SUCCESS)
            return retval;
    }
